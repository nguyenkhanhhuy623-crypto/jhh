...
-- Nâng cấp: key random, toggle menu, tele + auto spam F sau 0.5s, spam vừa phải
...
-- Xoá GUI cũ nếu có
local oldKey = PlayerGui:FindFirstChild("KeySys")
if oldKey then oldKey:Destroy() end
local oldMain = PlayerGui:FindFirstChild("Sinoracketeerin")
...
-- =========================================================
-- PHÍM TẮT TOGGLE MENU NGẪU NHIÊN
-- =========================================================
local keyCodes = {}
...
KeySystemFrame.Size = UDim2.new(0, 360, 0, 180)  -- cao hơn để hiển thị key
KeySystemFrame.Position = UDim2.new(0.5, -180, 0.5, -90)
...
-- Hiển thị key (ẩn đi để không lộ, nhưng có thể hiện ở đây để debug, tôi sẽ hiển thị nhưng nhỏ)
local KeyDisplay = Instance.new("TextLabel", KeySystemFrame)
KeyDisplay.Size = UDim2.new(1, 0, 0, 18)
KeyDisplay.Position = UDim2.new(0, 0, 0, 34)
KeyDisplay.BackgroundTransparency = 1
KeyDisplay.Font = Enum.Font.Gotham
KeyDisplay.TextSize = 12
KeyDisplay.Text = "Your key: " .. secretKey
...
KeyInput.Position = UDim2.new(0.06, 0, 0, 58)
...
local CopyKeyButton = Instance.new("TextButton", KeySystemFrame)
CopyKeyButton.Size = UDim2.new(0.44, -6, 0, 36)
CopyKeyButton.Position = UDim2.new(0.06, 0, 0, 106)
...
MainFrame.Size = UDim2.new(0, 330, 0, 400)  -- thêm chiều cao
...
-- Title color animation (dùng vòng lặp)
task.spawn(function()
    local hue = 0.0025
    while true do
        MainTitle.TextColor3 = Color3.fromHSV(hue, 0.95, 1)
        hue = hue + 0.0025
        if hue > 0.235 then hue = 0.0025 end
        task.wait(0.03)
    end
end)
...
-- =========================================================
-- CLOSE BUTTON (chỉ ẩn, không destroy)
-- =========================================================
...
-- =========================================================
-- TELEPORT (Teleport + tự động bật Spam F sau 0.5s)
-- =========================================================
...
TeleportButton.Text = "Teleport + Auto Spam F (0.5s)"
...
function startSpamF()
    ...
    spamFThread = task.spawn(function()
        while SpamFEnabled do
            ...
            task.wait(0.1)  -- tổng 0.15s, vừa phải
        end
        spamFThread = nil
    end)
end
...
TeleportButton.MouseButton1Click:Connect(function()
    ...
    -- Teleport lên 20 đơn vị (vừa phải)
    local pos = root.Position
    root.CFrame = CFrame.new(pos.X, pos.Y + 20, pos.Z)
    print("[Sinoracketeerin] Teleported up.")
    -- Sau 0.5s bật Spam F (nếu chưa bật)
    task.delay(0.5, function()
        if not SpamFEnabled then
            SpamFEnabled = true
            startSpamF()
            ...
            print("[Sinoracketeerin] Spam F already enabled.")
        end
    end)
end)
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.KeyCode == toggleKey then
        if MainGui.Enabled == false then
            -- Nếu menu đang ẩn và key system đã tắt (đã nhập key), thì hiện menu
            if not KeySystemGui.Enabled then
                MainGui.Enabled = true
                print("[Sinoracketeerin] Menu shown.")
            end
        else
            MainGui.Enabled = false
            print("[Sinoracketeerin] Menu hidden.")
...
-- KHỞI TẠO HOÀN TẤT
-- =========================================================
print("[Sinoracketeerin] Loaded successfully. Key: " .. secretKey .. ", Toggle key: " .. toggleKey.Name)