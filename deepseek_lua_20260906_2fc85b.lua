-- Deobf by Hihi (@reknoname) – sửa lỗi toàn bộ
...
-- =========================================================
-- TẠO SCREENGUI (không dùng BillboardGui để tránh lỗi)
-- =========================================================
...
-- =========================================================
-- HÀM TELEPORT ĐẾN "Dan cong" (tìm kiếm kỹ hơn)
-- =========================================================
local function teleportToDanCong()
    ...
    -- Tìm trong Workspace tất cả các đối tượng có tên chứa "Dan cong" (không phân biệt hoa thường)
    local target = nil
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj.Name and string.find(string.lower(obj.Name), "dan cong") then
            target = obj
            break
        end
    end
    ...
    if not target then
        print("[Sinoracketeerin] Không tìm thấy 'Dan cong' trong Workspace. Kiểm tra tên chính xác.")
        return
    end
    ...
    elseif target:IsA("Model") then
        -- Lấy trung bình vị trí của các Part con
        ...
        if count > 0 then
            ...
            print("[Sinoracketeerin] Model không có Part nào.")
            return
        end
    else
        print("[Sinoracketeerin] Đối tượng không phải Part hay Model hợp lệ.")
        return
    end

    -- Teleport
    hrp.CFrame = CFrame.new(targetPos)
    print("[Sinoracketeerin] Đã teleport đến 'Dan cong' tại:", targetPos)
end
